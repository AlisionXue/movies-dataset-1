...
# 在初次加载时，将 st.query_params 存储到 session state 中
if "stored_query_params" not in st.session_state:
    st.session_state["stored_query_params"] = st.query_params

# 从 session state 中获取查询参数
stored_params = st.session_state["stored_query_params"]
url_username = stored_params.get("username", [None])[0]
url_password = stored_params.get("password", [None])[0]

# 仅记录用户名，避免在日志中记录密码
logging.info(f"Stored query parameters - username: {url_username}")

# Predefined username and password
USERNAME = "admin"
PASSWORD = "123"

# Automatic login based on stored query parameters
